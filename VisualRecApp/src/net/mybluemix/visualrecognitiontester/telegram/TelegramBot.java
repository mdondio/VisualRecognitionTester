...
/**
 * This class handles all interactions between users and Telegram Bot.
 * Hardcoded: using Marco bot (DondiBot)
 * 
 * @author marcodondio
 *
 */
...
public class TelegramBot extends HttpServlet {
	...
	private String handleUpdate(JsonObject update) throws IOException {
		...
		String text = "Ciao " + (first_name == null ? "" : first_name.getAsString()) + " "
				+ (last_name == null ? "" : last_name.getAsString()) + "! :)";

		return sendMessage(chat_id, text);

		// //----------------------------------------------------------
		// // Se non mi va bene il default e lo devo modificare:
		// JsonArray arr = gson.toJsonTree(db.findByIndex(selector,
		// Dataset.class, o)).getAsJsonArray();
		////
		...
		// int size = images.getAsJsonArray("positive").size() +
		// images.getAsJsonArray("negative").size();
		//
		// obj.remove("images");
